...
```
  ...
  + `get methods`: methods to get any slot outside the object. In the case
    ...
    order to obtain fully functional datasets.
...
```
  ...
  + `assignation`: methods for `"<-"` are implemented, allowing transformation
    of the slots, i.e. when tranforming units.
...
objects:
  ...
  + `sfn_plot`. This function allows to plot `sfn_data` objects:
  
```{r sfn_plot, fig.show='hold', fig.width=3.4}
library(ggplot2)
...
```
  ...
  + `*_metrics` functions. Family of functions to aggregate and summarise the
    site data. See `?metrics` for detailed info.
