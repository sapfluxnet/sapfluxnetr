...
```
`sapfluxnetr` package offers a very flexible but powerful API absed on the
...
make use of the `sfn_metrics` funcion under the hood. If you want full control
...
use, but if you want simpler metrics and/or different metrics you can supply
your own summarising functions using the `.funs` argument.  
The correct way of especifying the functions are as described in the
`summarise_all` help (`help(dplyr::summarise_all)`), function names, quosure
style lambda functions or a list of either form. We **recommend** a list of
quosure style lamda functions, as is easier to add the corresponding arguments
for each function:
...
```
 ...
 + A custom function name (without quotes). This way you can build irregular
   ...
   can have other arguments that will be supplied in the dots (`...`) argument
