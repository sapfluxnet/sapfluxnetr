...
`vignette('02_sfn_data_classes', package = 'sapfluxnetr'` for detailed info):
...
`vignette('02_sfn_data_classes', package = 'sapfluxnetr'` for more info.
...
```{r exmaple_1, fig.width=7, fig.height=4.3}
...
env_data <- temp_hr_aggregated %>%
  map(c('env', 'env_gen')) %>%
  map(~ select(., TIMESTAMP, ends_with('_mean'))) %>%
  map(~ select(., TIMESTAMP, starts_with('vpd_')))

plant_md <- sfn_metadata[['plant_md']] %>%
  filter(si_code %in% names(temp_hr_aggregated))

whole_data <- sapf_data %>%
  map2(env_data, ~ full_join(.x, .y, by = 'TIMESTAMP')) %>%
  map(~ gather(., Tree, Value, -TIMESTAMP, -starts_with('vpd_'))) %>%
  map(as_tibble) %>%
  multi_union() %>%
  mutate(pl_code = str_remove_all(Tree, '_q_95')) %>%
  arrange(TIMESTAMP) %>%
  left_join(plant_md, by = 'pl_code')


ggplot(whole_data, aes(x = vpd_mean, y = Value, colour = si_code,
                       size = pl_sapw_area)) +
