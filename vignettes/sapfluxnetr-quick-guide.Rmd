...
`sapfluxnetr` stable versions will be in the CRAN repository, so the
installation of the package is as per usual:
...
`daily_metrics` and related functions also implement the `tidy` argument. This
allows for returning directly a metrics data frame combining all sites
