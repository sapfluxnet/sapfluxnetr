...
`sapfluxnetr` is not yet in CRAN, so the installation must be done with the
[remotes](https://github.com/r-lib/remotes#readme) package:

```{r remotes_inst, eval=FALSE}
...
During the embargo period (2019-03-05 to 2020-03-05) only the data contributors
can access the data. If you are a data contributor willing to access the data,
follow these steps:

  1. Go to SAPFLUXNET repository in Zenodo (https://zenodo.org/record/2530798)
  1. Click in the `Request access` button, fill the form and click the
     `Send request` button.
  1. SAPFLUXNET Team will send you an email with the link to activate your
     access to the Zenodo repository.
  1. Follow the link and click in the download button. This will download a
     zip file
  1. Unzip the file in any location
...
To start working with the data, you have two options:
  ...
  - If working in RStudio, create a project in the data root folder (`0.1.3` in
    this example) and follow the examples in this vignette to get started.
  
  - If not, set the data root folder (`0.1.3` in this example) as the working
...
SAPFLUXNET data is stored as subdaily measures with different timestep between
sites (ranging from 10 minutes to 2 hours) for both, sap flow and environmental
data. Aggregation metrics are calculated for both kinds of data.  
...
Now we can start analysing, modelling, etc.  
